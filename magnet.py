...
class Agnes(object):
    ...
    def generate_dissimilarity(self):
        ...
        #	]
        col = []
        for i in range(len(self.clusterlist) - 1):
            row = []
            for j in range(len(self.clusterlist) - i - 1):
    ...
    def agnes_step(self):
        ...
        x, y = 0, self.dissimilarity[0].index(
            md)  # pozíciója a vektorban, FONTOS!!! ez nem egyenlő a clusterlistbeli pozíciójával, le kell képezni
...
def main():
    ...
    for i in range(len(clusterlist) - num_of_clusters):
        agnes.agnes_step()
